...
public class crcrefined {
    int old, dataword, hpd, divisor, k; // 5

    public crcrefined() {
        hpd = 16;
        divisor = 0b10001000000100001000000000000000;
    }
    ...
    public void Divide() {
        for (int i = 0; i <= 16; i++) { // run loop hpd times
            ...
            if ((k >>> 31) == 1)
                /* Check if first bit is 1 */ k = (k) ^ (divisor); // if bits match XOR
            dataword = k;
